...
- Focus on taking the user's request and augmenting it with "VHS, filmic, vaporwave, neon, cyberpunk, cybernetic, etc."
...
Enhanced prompt:`;
            ...
            if (!lumaApiKey) {
                elizaLogger.error(
                    "LUMA_API_KEY is not set in runtime settings"
                );
                await callback(
                    {
...
Your response can be as long as you want, even long paragraphs as long as they are interesting.`;
                        ...
                        elizaLogger.log(
                            "Successfully posted video to Twitter:",
                            tweetResult
                        );
                ...
                elizaLogger.error(
                    "Video generation failed with response:",
                    video
                );
                await callback(
                    {
